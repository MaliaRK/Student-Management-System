...
class StudentData {
    ...
    constructor(name: string, id: number, course_enrolled: any, fees: number, balance: number | string | null, status: string) {
        this.Name = name;
        this.ID = id;
    ...
    updateCourse(oldCourse: string, newCourse: string) {
...
let studentID: string = '';    // User-input student ID
const usedIDs: string[] = [];   // Unique student IDs to prevent duplicates
...
while (studentID.length !== 5 || isNaN(Number(studentID)) || usedIDs.includes(studentID)) {
    ...
    const student: StudentData = new StudentData(
        dataAnswer.name,
        parseInt(studentID),  // Convert id to number
        answer.course,
        Math.abs(answer.fees),   // Ensure fees is positive
